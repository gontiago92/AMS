...
buttons.forEach(button => {
    ...
    button.addEventListener('click', function(e) {
        ...
        if (command == 'createlink' || command == 'insertimage') {
            ...
            if(url === null) {
                alert('Please enter a valid url adress!')
                url = prompt('Enter the link here: ', 'http:\/\/');
            } else {
                document.execCommand(command, false, url)
            }
        } else if (command == 'insertHTML') {
            instaPost = prompt('Enter link to the instagram post here: ', 'https:\/\/')
            url = `<blockquote class="instagram-media" data-instgrm-captioned="" data-instgrm-permalink="` + instaPost + `?utm_source=ig_embed&amp;utm_campaign=loading" data-instgrm-version="12" style="background:#FFF; border:0; border-radius:3px; box-shadow:0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15); margin: 1px; max-width:540px; min-width:326px; padding:0; width:99.375%; width:-webkit-calc(100% - 2px); width:calc(100% - 2px);">
        <div style="padding:16px;">
          <a href="` + instaPost + `?utm_source=ig_embed&amp;utm_campaign=loading" style="background:#FFFFFF; line-height:0; padding:0 0; text-align:center; text-decoration:none; width:100%;" target="_blank">
